...
const updateAvatarURL = async(userId, avatarUrl) => {
    try {
        const user = await UserModel.findOneAndUpdate({ _id: userId }, { avatar: avatarUrl }, { new: true });
        console.log(user);
        return user;
    } catch (error) {
...
const addNewFriend = async(userId, friendId) => {
    ...
    try {
        ...
        const user = await UserModel.findOneAndUpdate({ _id: userId, friends: { $ne: friendId } }, { $addToSet: { friends: friendId } },
            opts
        );
        if (!user) {
        ...
        const friend = await UserModel.findOneAndUpdate({ _id: friendId, friends: { $ne: userId } }, { $addToSet: { friends: userId } },
            opts
        );
        if (!friend) {
            await session.abortTransaction();
...
const deleteFriendById = async(userId, friendId) => {
    try {
        const user = await UserModel.findOneAndUpdate({ _id: userId, friends: friendId }, { $pull: { friends: friendId } }, { new: true });
