...
const checkExistConversation = async (senderUserId, reciverUserId) => {
    let conversationList = await ConversationModel.find({
        isGroup: false,
        $and: [{ users: { $elemMatch: { $eq: senderUserId } } }, { users: { $elemMatch: { $eq: reciverUserId } } }],
    })
...
const populateConversation = async (conversationId, field, fieldRemove) => {
    const conversation = await ConversationModel.findOne({ _id: conversationId }).populate(field, fieldRemove);
    if (!conversation) {
        throw createHttpError.BadRequest('Some thing wrong, Try agian');
    }
    return conversation;
...
const getListUserConversations = async (userId) => {
    ...
    await ConversationModel.find({
        ...
        .populate('users', ['-password', '-qrCode', '-background', '-dateOfBirth', '-createdAt', '-updatedAt'])
        .populate('admin', ['-password', '-qrCode', '-background', '-dateOfBirth', '-createdAt', '-updatedAt'])
        .populate('lastMessage')
        .sort({ updatedAt: -1 })
        .then(async (data) => {
...
const updateLastMessage = async (conversationId, message) => {
    const conversationUpdated = await ConversationModel.findByIdAndUpdate(conversationId, { lastMessage: message });
